...
always @ (posedge clk_sample) begin
    ...
    wire Condition_For_Axis = ((VGA_HORZ_COORD == 640) && (VGA_VERT_COORD <= 512)) || (VGA_VERT_COORD == 256);

// The code below draws two grid lines. Modify the codes to draw more grid lines.

    wire Condition_For_Grid = (VGA_VERT_COORD <= 512) && ((VGA_HORZ_COORD % 80 == 0) ||(VGA_VERT_COORD % 64 == 0)) ;

// Using the gridline example, insert your code below to draw ticks on the x-axis and y-axis.

    wire Condition_For_Ticks = ((VGA_VERT_COORD % 16 == 0) && (VGA_VERT_COORD <= 512) && (VGA_HORZ_COORD < 646) && (VGA_HORZ_COORD > 634)) || ((VGA_HORZ_COORD % 16 == 0) && (VGA_VERT_COORD < 262) && (VGA_VERT_COORD > 250));
    wire colour = (VGA_HORZ_COORD % 3 == 0);
...
// Please modify below codes to change the background color and to display ticks defined above
    ...
    assign VGA_Red_Grid = ((VGA_HORZ_COORD < 1280) &&(VGA_VERT_COORD == (1024 - Sample_Memory[VGA_HORZ_COORD])))? 0 :Condition_For_Axis ? axis[0] : Condition_For_Ticks ? tick[0] : Condition_For_Grid? grid[0] : ((VGA_VERT_COORD >= 1024 - mem[VGA_HORZ_COORD]) && colour)? 4'hd : color_select[11:8];

    assign VGA_Green_Grid = ((VGA_HORZ_COORD < 1280) &&(VGA_VERT_COORD == (1024 - Sample_Memory[VGA_HORZ_COORD])))? 0 :Condition_For_Axis ? axis[1] : Condition_For_Ticks ? tick[1] : Condition_For_Grid? grid[1] : ((VGA_VERT_COORD >= 1024 - mem[VGA_HORZ_COORD]) && colour )? 4'hd : color_select[11:8];

    assign VGA_Blue_Grid = ((VGA_HORZ_COORD < 1280) &&(VGA_VERT_COORD == (1024 - Sample_Memory[VGA_HORZ_COORD])) && VGA_VERT_COORD >= 512)? 0 :Condition_For_Axis ? axis[2] : Condition_For_Ticks ? tick[2] : Condition_For_Grid? grid[2] : ((VGA_VERT_COORD >= 1024 - mem[VGA_HORZ_COORD]) && colour)? 4'hd : color_select[11:8];
